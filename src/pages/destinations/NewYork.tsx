...
const hotels = [
  { id: 'n1', name: 'The Plaza Hotel', stars: 5, price: 210, rating: 9.5, reviews: 3120, img: '/chrice-agency/assets/ny_1.jpg', desc: "L'institution new-yorkaise par excellence face à Central Park.", amenities: ['Vue Central Park', 'Sèche-cheveux Dyson', 'Literie Frette'] },
  { id: 'n2', name: 'YOTEL New York', stars: 4, price: 110, rating: 8.9, reviews: 5400, img: '/chrice-agency/assets/ny_2.jpg', desc: "Design intelligent et moderne en plein cœur de Manhattan à prix doux.", amenities: ['Smartbed ajustable', 'Douche mousson', 'Rooftop panoramique'] },
  { id: 'n3', name: 'Arlo SoHo', stars: 4, price: 140, rating: 9.1, reviews: 2200, img: '/chrice-agency/assets/ny_3.jpg', desc: "Micro-hôtel design avec un esprit boutique au cœur du bouillonnant SoHo.", amenities: ['Vélos gratuits', 'Cour intérieure privée', 'Produits apothicaires'] },
  { id: 'n4', name: 'citizenM Times Square', stars: 4, price: 130, rating: 9.3, reviews: 4800, img: '/chrice-agency/assets/ny_4.jpg', desc: "Un luxe abordable pour les voyageurs modernes à deux pas de Times Square.", amenities: ['Contrôle via iPad', 'Films gratuits', 'Lobby façon salon design'] }
];

const excursions = [
  { id: 'e1', name: 'Pass VIP Empire State Building (Coupe-file)', price: 45 },
  { id: 'e2', name: 'Vol en Hélicoptère au-dessus de Manhattan', price: 195 },
  { id: 'e3', name: 'Billet Comédie Musicale Broadway', price: 110 }
];
...
export const NewYork = () => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--color-brand-500', '#64748b');
    document.documentElement.style.setProperty('--color-brand-400', '#94a3b8');
    return () => {
  ...
  const baseFlightPrice = tripType === 'aller-retour' ? 320 : 180;
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const customFlight: Flight = {
      ...flight, arrival: { ...flight.arrival, code: 'JFK' }, departure: { ...flight.departure, code: userCity.substring(0,3).toUpperCase() },
      price: Math.round(totalPrice) 
    };
    
    setSearchParams({ from: `${userCity} (Vol + Hôtel)`, to: 'New York, USA', date: tripType === 'aller-retour' ? `${dateAller} au ${dateRetour}` : dateAller, passengers });
  ...
  return (
    <div className="w-full">
      <div className="relative h-[60vh] overflow-hidden mb-16 shadow-2xl">
        <img src="/chrice-agency/assets/dl_asset_1781709136346_2.jpg" alt="New York" className="w-full h-full object-cover brightness-50" />
      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-16">
        <div className="flex-1">
          <h2 className="text-4xl font-bold mb-6 text-white">Rêve Américain</h2>
          <p className="text-xl text-slate-300 leading-relaxed mb-16">Des gratte-ciels époustouflants aux lumières de Broadway, vivez l'énergie unique de Manhattan avec des prix exclusifs négociés pour vous.</p>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select value={mealPlan} onChange={(e) => setMealPlan(Number(e.target.value))} className="w-full glass rounded-xl px-4 py-3 text-white font-bold outline-none [color-scheme:dark] appearance-none cursor-pointer">
                  <option value={0}>Café et Viennoiserie le matin (+0)</option>
                  <option value={35}>Petit-déjeuner buffet Américain (+{formatPrice(35)}/j/pers)</option>
                  <option value={75}>Formule Diner Broadway (+{formatPrice(75)}/j/pers)</option>
                </select>
              </div>
