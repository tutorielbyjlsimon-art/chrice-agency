...
const excursions = [
  { id: 'e1', name: 'Safari privé dans le désert & Dîner Bédouin', price: 85 },
  { id: 'e2', name: 'Billet VIP Burj Khalifa (148ème étage)', price: 110 },
  { id: 'e3', name: 'Location de Yacht privé (2h)', price: 200 }
];
...
export const Dubai = () => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--color-brand-500', '#eab308');
    document.documentElement.style.setProperty('--color-brand-400', '#facc15');
    return () => {
  ...
  const baseFlightPrice = tripType === 'aller-retour' ? 280 : 150;
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const customFlight: Flight = {
      ...flight, arrival: { ...flight.arrival, code: 'DXB' }, departure: { ...flight.departure, code: userCity.substring(0,3).toUpperCase() },
      price: Math.round(totalPrice) 
    };
    
    setSearchParams({ from: `${userCity} (Vol + Hôtel)`, to: 'Dubaï, EAU', date: tripType === 'aller-retour' ? `${dateAller} au ${dateRetour}` : dateAller, passengers });
  ...
  return (
    <div className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-8 mb-12 relative z-10">
        <LiveLocalStatus city="Dubaï" timezone="Asia/Dubai" weather="sun" temp={35} />
      </div>

      <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-16">
        <div className="flex-1">
          <h2 className="text-4xl font-bold mb-6 text-white">Le Luxe Futuriste</h2>
          <p className="text-xl text-slate-300 leading-relaxed mb-16">Où le désert rencontre l'océan. Profitez du shopping de classe mondiale, de l'architecture audacieuse et d'un luxe accessible exceptionnel.</p>
              ...
              <div key={h.id} onClick={() => setSelectedHotel(h.id)} className={`glass-card overflow-hidden cursor-pointer transition-all border-2 ${selectedHotel === h.id ? 'border-brand-500 shadow-[0_0_20px_rgba(205,162,96,0.3)] scale-[1.02]' : 'border-transparent hover:border-white/20'}`}>
                ...
                <div className="p-5">
                  ...
                  <div className="flex gap-2 items-center mb-3 flex-wrap">
                    ...
                    <span className="bg-blue-500/20 text-blue-400 text-xs font-bold px-2 py-1 rounded-md flex items-center gap-1">Wifi Ultra-Rapide</span>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select value={mealPlan} onChange={(e) => setMealPlan(Number(e.target.value))} className="w-full glass rounded-xl px-4 py-3 text-white font-bold outline-none [color-scheme:dark] appearance-none cursor-pointer">
                  <option value={0}>Petit-déjeuner buffet inclus (+0)</option>
                  <option value={45}>Demi-pension Internationale (+{formatPrice(45)}/j/pers)</option>
                  <option value={90}>All Inclusive Premium (+{formatPrice(90)}/j/pers)</option>
                </select>
              </div>
