...
const excursions = [
  { id: 'e1', name: 'Cours de natation VIP & Plongée sous-marine', price: 120 },
  { id: 'e2', name: 'Croisière privée au coucher du soleil avec dauphins', price: 85 },
  { id: 'e3', name: 'Dîner exclusif sur un banc de sable désert', price: 190 }
];
...
export const Maldives = () => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--color-brand-500', '#0ea5e9');
    document.documentElement.style.setProperty('--color-brand-400', '#38bdf8');
    return () => {
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const customFlight: Flight = {
      ...flight, arrival: { ...flight.arrival, code: 'MLE' }, departure: { ...flight.departure, code: userCity.substring(0,3).toUpperCase() },
      price: Math.round(totalPrice) 
    };
    
    setSearchParams({ from: `${userCity} (Vol + Hôtel)`, to: 'Maldives', date: tripType === 'aller-retour' ? `${dateAller} au ${dateRetour}` : dateAller, passengers });
    ...
    <div className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-16">
        <div className="flex-1">
          <h2 className="text-4xl font-bold mb-6 text-white">Le Paradis sur Terre</h2>
          <p className="text-xl text-slate-300 leading-relaxed mb-16">L'archipel des Maldives est la destination ultime pour déconnecter. Eau cristalline, villas sur pilotis et sable d'une blancheur immaculée.</p>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select value={mealPlan} onChange={(e) => setMealPlan(Number(e.target.value))} className="w-full glass rounded-xl px-4 py-3 text-white font-bold outline-none [color-scheme:dark] appearance-none cursor-pointer">
                  ...
                  <option value={110}>Formule "All Inclusive" (+{formatPrice(110)}/j/pers)</option>
                </select>
              </div>
