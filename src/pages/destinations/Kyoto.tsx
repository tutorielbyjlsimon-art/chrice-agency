...
const hotels = [
  { id: 'k1', name: 'Ritz-Carlton Kyoto', stars: 5, price: 250, rating: 9.7, reviews: 815, img: '/chrice-agency/assets/kyoto_1.jpg', desc: "Design exceptionnel au bord de la rivière Kamogawa, fusionnant tradition et luxe absolu.", amenities: ['Bain Onsen intérieur privé', 'Produits de bain Asprey', 'Literie sur-mesure'] },
  { id: 'k2', name: 'Aman Kyoto', stars: 5, price: 400, rating: 9.9, reviews: 312, img: '/chrice-agency/assets/kyoto_2.jpg', desc: "Un sanctuaire zen caché dans une forêt secrète. L'expérience Ryokan la plus exclusive du Japon.", amenities: ['Tatami traditionnel premium', 'Jardin Zen privatif', 'Immersion nature 360°'] },
  { id: 'k3', name: 'Suiran, Luxury Collection', stars: 5, price: 180, rating: 9.4, reviews: 620, img: '/chrice-agency/assets/kyoto_3.jpg', desc: "Une retraite intime nichée dans les collines d'Arashiyama, avec bains thermaux privés.", amenities: ['Bain thermal extérieur', 'Architecture bois et papier', 'Cérémonie du thé'] },
  { id: 'k4', name: 'Four Seasons Kyoto', stars: 5, price: 220, rating: 9.6, reviews: 450, img: '/chrice-agency/assets/kyoto_4.jpg', desc: "Un jardin ancestral de 800 ans au cœur de l'hôtel, offrant une sérénité inégalable.", amenities: ['Piscine intérieure 20m', 'Chambre vue sur le lac', 'Technologie intégrée invisible'] }
];

const excursions = [
  { id: 'e1', name: 'Massage traditionnel Shiatsu (90min)', price: 95 },
  { id: 'e2', name: 'Visite VIP guidée de la forêt de bambous', price: 65 },
  { id: 'e3', name: 'Dîner privé avec une authentique Maiko', price: 180 }
];
...
export const Kyoto = () => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--color-brand-500', '#ef4444');
    document.documentElement.style.setProperty('--color-brand-400', '#f87171');
    return () => {
  ...
  const baseFlightPrice = tripType === 'aller-retour' ? 350 : 200;
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const customFlight: Flight = {
      ...flight, arrival: { ...flight.arrival, code: 'KYO' }, departure: { ...flight.departure, code: userCity.substring(0,3).toUpperCase() },
      price: Math.round(totalPrice) 
    };
    
    setSearchParams({ from: `${userCity} (Vol + Hôtel)`, to: 'Kyoto, Japon', date: tripType === 'aller-retour' ? `${dateAller} au ${dateRetour}` : dateAller, passengers });
  ...
  return (
    <div className="w-full">
      <div className="relative h-[60vh] overflow-hidden mb-16 shadow-2xl">
        <img src="/chrice-agency/assets/dest_kyoto_1781704278607.jpg" alt="Kyoto" className="w-full h-full object-cover brightness-50" />
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-8 mb-12 relative z-10">
        <LiveLocalStatus city="Kyoto" timezone="Asia/Tokyo" weather="cloud" temp={18} />
      </div>

      <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-16">
        <div className="flex-1">
          <h2 className="text-4xl font-bold mb-6 text-white">L'Harmonie Spirituelle</h2>
          <p className="text-xl text-slate-300 leading-relaxed mb-16">Découvrez l'ancienne capitale impériale. Kyoto est la quintessence du Japon traditionnel, mêlant temples dorés, jardins zen immaculés et sanctuaires secrets.</p>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select value={mealPlan} onChange={(e) => setMealPlan(Number(e.target.value))} className="w-full glass rounded-xl px-4 py-3 text-white font-bold outline-none [color-scheme:dark] appearance-none cursor-pointer">
                  ...
                  <option value={45}>Demi-pension traditionnelle (+{formatPrice(45)}/j/pers)</option>
                  <option value={90}>Pension complète Kaiseki (+{formatPrice(90)}/j/pers)</option>
                </select>
              </div>
